      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredAssets.length === 0 ? (
          ...
          filteredAssets.map((ast) => (
            <div
              key={ast.id}
              className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-4 hover:border-teal-500/40 transition-all shadow-md flex flex-col justify-between"
            >
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-teal-500/10 text-teal-300 border border-teal-500/20 font-mono">
                      {ast.assetCode}
                    </span>
                    <h3 className="text-base font-bold text-white mt-1.5">{ast.name}</h3>
                    <p className="text-xs text-slate-400">{ast.category}</p>
                  </div>
                ...
                <div className="space-y-1.5 text-xs text-slate-300 pt-2 border-t border-slate-800">
                  <div className="flex justify-between">
                    <span className="text-slate-400">{isBangla ? "সিরিয়াল নম্বর:" : "Serial Number:"}</span>
                    <span className="font-mono text-slate-200">{ast.serialNumber}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">{isBangla ? "ক্রয়মূল্য:" : "Asset Value:"}</span>
                    <span className="font-semibold text-emerald-400 font-mono">
                      ৳{(ast.purchaseCost ?? 0).toLocaleString()} BDT
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400">{isBangla ? "কন্ডিশন পরিবর্তন:" : "Change Condition:"}</span>
                    <select
                      value={ast.condition}
                      onChange={(e) => {
                        if (onUpdateAssetCondition) {
                      ...
                      className="bg-slate-950 border border-slate-700 text-slate-200 text-[10px] rounded px-1.5 py-0.5"
                    >
                ...
                <div className="p-3 rounded-xl bg-slate-800/60 flex items-center justify-between text-xs">
                  <div>
                    <span className="text-[10px] text-slate-400 block">{isBangla ? "বরাদ্দকৃত কর্মী" : "Assigned Custodian"}</span>
              ...
              <div className="pt-3 border-t border-slate-800 flex items-center justify-between gap-2">
                {ast.assignedToEmployeeId ? (
                  <button
                    ...
                    className="flex-1 py-1.5 px-3 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 font-bold text-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 py-1.5 px-3 rounded-xl bg-teal-500/10 hover:bg-teal-500/20 text-teal-300 border border-teal-500/30 font-bold text-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md text-slate-100 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleCreateAsset} className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-400 mb-1">{isBangla ? "অ্যাসেট নাম ও মডেল *" : "Asset Name & Model *"}</label>
                <input
                  type="text"
                  value={newName}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-slate-400 mb-1">{isBangla ? "ক্যাটাগরি" : "Category"}</label>
                  <select
                    value={newCategory}
                ...
                <div>
                  <label className="block text-slate-400 mb-1">{isBangla ? "সিরিয়াল নম্বর" : "Serial Number"}</label>
                  <input
                    type="text"
                    value={newSerial}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-slate-400 mb-1">{isBangla ? "ক্রয়মূল্য (৳)" : "Purchase Cost (৳)"}</label>
                  <input
                    type="number"
                    value={newCost}
                ...
                <div>
                  <label className="block text-slate-400 mb-1">{isBangla ? "বর্তমান কন্ডিশন" : "Initial Condition"}</label>
                  <select
                    value={newCondition}
              ...
              <div>
                <label className="block text-slate-400 mb-1">{isBangla ? "বরাদ্দ দিন (ঐচ্ছিক)" : "Assign Custodian (Optional)"}</label>
                <select
                  value={newAssigneeId}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md text-slate-100 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              ...
              <button onClick={() => setShowWithdrawModal(null)} className="text-slate-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-xs space-y-1">
              <div className="font-bold text-white">{showWithdrawModal.name} ({showWithdrawModal.assetCode})</div>
              <div className="text-slate-400">
                {isBangla ? "বর্তমান ব্যবহারকারী:" : "Current Custodian:"}{" "}
                <span className="text-teal-300 font-bold">{showWithdrawModal.assignedToEmployeeName}</span>
              </div>
            </div>
            ...
            <form onSubmit={handleWithdrawSubmit} className="space-y-3 text-xs">
              ...
              <div>
                <label className="block text-slate-400 mb-1">{isBangla ? "মন্তব্য / নোট" : "Return Clearance Notes"}</label>
                <textarea
                  rows={2}
                  value={withdrawNote}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md text-slate-100 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              ...
              <button onClick={() => setShowReassignModal(null)} className="text-slate-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-xs">
              <div className="font-bold text-white">{showReassignModal.name} ({showReassignModal.assetCode})</div>
            </div>
