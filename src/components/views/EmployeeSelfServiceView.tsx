...
export const EmployeeSelfServiceView: React.FC<EmployeeSelfServiceViewProps> = ({
  ...
  return (
    <div id="employee-self-service-portal" className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="p-5 sm:p-6 rounded-3xl bg-gradient-to-r from-slate-900 via-slate-850 to-slate-900 border border-slate-800 shadow-md flex flex-col xl:flex-row items-center xl:items-center justify-between gap-5">
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-5 text-center sm:text-left w-full xl:w-auto">
          <div className="relative shrink-0">
            <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-2xl overflow-hidden border-2 border-teal-500 shadow-xl shadow-teal-500/20 bg-slate-800 flex items-center justify-center">
              <img
            ...
            <div className="absolute -bottom-2 -right-2 bg-slate-900 border border-slate-700 p-1.5 rounded-xl shadow-lg">
          ...
          <div className="space-y-1 w-full sm:w-auto">
            ...
            <p className="text-xs sm:text-sm font-semibold text-teal-400">
              {currentEmployee.designationTitle} • {currentEmployee.departmentName}
            </p>

            <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs text-slate-400 justify-center sm:justify-start mt-1">
              <span>Employee Code: <strong className="text-white">{currentEmployee.employeeCode}</strong></span>
              <span className="hidden sm:inline">•</span>
              <span>Branch: <strong className="text-white">{currentEmployee.branchName}</strong></span>
              <span className="hidden sm:inline">•</span>
              <span>Shift: <strong className="text-teal-300">{currentEmployee.shiftName}</strong></span>
            </div>
          </div>
        </div>

        {/* Action Buttons Toolbar - Tablet & Mobile Resilient */}
        <div className="flex flex-wrap items-center justify-center sm:justify-start xl:justify-end gap-2.5 w-full xl:w-auto pt-2 xl:pt-0 border-t border-slate-800/80 xl:border-t-0">
          {onOpenDigitalIdCard && (
            <button
              type="button"
              id="ess-digital-id-btn"
              onClick={onOpenDigitalIdCard}
              className="px-3.5 py-2.5 bg-gradient-to-r from-teal-500/15 via-emerald-500/15 to-teal-500/15 hover:from-teal-500/25 hover:to-emerald-500/25 border border-teal-500/40 text-teal-300 text-xs font-bold rounded-xl shadow-xs flex items-center gap-2 transition-all cursor-pointer grow sm:grow-0 justify-center"
            >
          ...
          <button
            ...
            className="px-3.5 py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white text-xs font-bold rounded-xl shadow-xs flex items-center gap-2 transition-all cursor-pointer grow sm:grow-0 justify-center"
          >
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-2 overflow-x-auto text-xs font-bold scrollbar-thin scrollbar-thumb-slate-700">
        {[
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-white">Biometric Face & Device Binding</h3>
            </div>

            <div className="space-y-3 text-xs">
              <div className="p-3.5 rounded-xl bg-slate-800/60 border border-slate-700/60 flex items-center justify-between">
                <div>
                  <p className="font-semibold text-slate-200">Face Vector Template</p>
              ...
              <button
                ...
                className="w-full py-2.5 px-3 bg-teal-500/15 hover:bg-teal-500/25 border border-teal-500/40 text-teal-300 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all shadow-sm cursor-pointer"
              >
              ...
              <div className="p-3.5 rounded-xl bg-slate-800/60 border border-slate-700/60 flex items-center justify-between">
                <div>
                  <p className="font-semibold text-slate-200">Bound Work Device</p>
              ...
              <div className="p-3.5 rounded-xl bg-slate-800/60 border border-slate-700/60 flex items-center justify-between">
                <div>
                  <p className="font-semibold text-slate-200">Geofence Compliance</p>
                  <p className="text-[10px] text-slate-400">Within allowed branch perimeter</p>
                </div>
          ...
          <div className="lg:col-span-2 p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
            <h3 className="text-sm font-bold text-white">Employee Personal & Corporate Record</h3>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/50">
                <span className="text-slate-400 block text-[11px]">Official Email</span>
                <span className="font-bold text-slate-200">{currentEmployee.email}</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/50">
                <span className="text-slate-400 block text-[11px]">Contact Phone</span>
                <span className="font-bold text-slate-200">{currentEmployee.phone}</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/50">
                <span className="text-slate-400 block text-[11px]">Date of Joining</span>
                <span className="font-bold text-slate-200">{currentEmployee.joiningDate}</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/50">
                <span className="text-slate-400 block text-[11px]">National ID (NID)</span>
                <span className="font-bold text-slate-200">{currentEmployee.nidNumber}</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/50">
                <span className="text-slate-400 block text-[11px]">Blood Group</span>
                <span className="font-bold text-red-400">{currentEmployee.bloodGroup}</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/50">
              ...
              <div className="sm:col-span-2 p-3 rounded-xl bg-slate-800/40 border border-slate-700/50">
                <span className="text-slate-400 block text-[11px]">Present Residential Address</span>
                <span className="font-medium text-slate-200">{currentEmployee.presentAddress}</span>
              </div>
            </div>

            {/* Digital ID Card Action Banner */}
            {onOpenDigitalIdCard && (
              <div className="p-4 rounded-xl bg-gradient-to-r from-teal-950/60 via-slate-850 to-slate-900 border border-teal-500/30 flex flex-col sm:flex-row items-center justify-between gap-4 mt-4">
      ...
      {activeSubTab === "attendance" && (
        <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-white">My Biometric & GPS Attendance Logs</h3>
            <button
              onClick={onOpenAttendanceModal}
              ...
              <tbody className="divide-y divide-slate-800 text-slate-300">
                {myAttendance.map((rec) => (
                  <tr key={rec.id} className="hover:bg-slate-800/40">
                    ...
                    <td className="p-3 font-mono text-emerald-400 font-bold">{rec.checkInTime}</td>
                    <td className="p-3 font-mono text-blue-400">{rec.checkOutTime || "Active"}</td>
                    <td className="p-3">
      ...
      {activeSubTab === "payslips" && (
        <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-white">My Monthly Payslips & Breakdown</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {myPayslips.map((slip) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-xs font-bold text-white">{slip.payrollMonth} Payslip</span>
                    <p className="text-[10px] text-slate-400">{slip.paymentDate || "Processed"}</p>
                  </div>
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                    {slip.paymentStatus}
                  </span>
                ...
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between text-slate-300">
                    ...
                    <span className="font-semibold text-white">৳{(slip.grossEarnings ?? 0).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-slate-400">
                    <span>Total Deductions (PF, Tax, Late):</span>
                    <span className="text-red-400">-৳{(slip.totalDeductions ?? 0).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-slate-200 font-bold pt-2 border-t border-slate-700">
                    <span>Net Disbursed Salary:</span>
                    <span className="text-emerald-400 text-sm">৳{(slip.netSalary ?? 0).toLocaleString()}</span>
                  </div>
                </div>

                <button
                  onClick={() => onViewPayslip(slip)}
                  className="w-full py-2 bg-slate-700 hover:bg-slate-650 text-slate-100 text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 transition-colors"
                >
      ...
      {activeSubTab === "leaves" && (
        <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-white">My Leave Requests & Balance</h3>
            <button
              onClick={() => setShowLeaveModal(true)}
              className="px-3 py-1.5 bg-teal-600 hover:bg-teal-500 text-white text-xs font-bold rounded-lg shadow flex items-center gap-1.5"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-center text-xs">
            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700/60">
              <span className="text-slate-400 block text-[11px]">Casual Leave Balance</span>
              <span className="text-lg font-black text-teal-400">10 / 14 Days</span>
            </div>
            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700/60">
              <span className="text-slate-400 block text-[11px]">Sick Leave Balance</span>
              <span className="text-lg font-black text-blue-400">12 / 14 Days</span>
            </div>
            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700/60">
              <span className="text-slate-400 block text-[11px]">Annual Earned Leave</span>
              <span className="text-lg font-black text-amber-400">15 Days</span>
            </div>
            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700/60">
              <span className="text-slate-400 block text-[11px]">Special Leave</span>
              <span className="text-lg font-black text-emerald-400">Allowed</span>
            </div>
          </div>

          <div className="space-y-3 pt-2">
            <h4 className="text-xs font-bold text-slate-300">My Leave Application History</h4>
            {myLeaves.map((l) => (
              <div
                key={l.id}
              ...
              >
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-white">{l.leaveType} Leave</span>
                    <span className="text-slate-400">
                      ({l.startDate} to {l.endDate} • {l.totalDays} Days)
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-300 mt-1 italic">"{l.reason}"</p>
                  {l.reviewerComments && (
      ...
      {activeSubTab === "loans" && (
        <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-white">My Active Loans & EMI Schedule</h3>
            <button
          ...
          <div className="space-y-3">
            {myLoans.map((loan) => (
              <div
                key={loan.id}
                className="p-5 rounded-xl bg-slate-800/60 border border-slate-700/80 space-y-3"
              >
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-bold text-white text-sm">৳{(loan.amount ?? 0).toLocaleString()} Loan</h4>
                ...
                <div className="grid grid-cols-3 gap-3 text-center text-xs pt-2 border-t border-slate-700">
                  <div className="p-2 rounded bg-slate-900">
                    <span className="text-slate-400 block text-[10px]">Monthly EMI</span>
                    <span className="font-bold text-white">৳{(loan.monthlyEmi ?? 0).toLocaleString()}</span>
                  </div>
                  <div className="p-2 rounded bg-slate-900">
                    <span className="text-slate-400 block text-[10px]">Paid Installments</span>
      ...
      {activeSubTab === "assets" && (
        <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
          <h3 className="text-sm font-bold text-white">Company Assets Assigned To Me</h3>
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h4 className="font-bold text-white text-sm mt-0.5">{ast.name}</h4>
                  </div>
                ...
                <div className="space-y-1 text-xs text-slate-300 pt-2 border-t border-slate-700">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-slate-200">{ast.serialNumber}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-emerald-400 font-semibold">{ast.condition}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      {showLeaveModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md space-y-4 text-slate-100 shadow-2xl">
            <h3 className="text-base font-bold text-white flex items-center gap-2">
              <CalendarCheck className="w-5 h-5 text-teal-400" /> Apply For Leave
            </h3>
            ...
            <form onSubmit={handleLeaveSubmit} className="space-y-3 text-xs">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-gradient-to-r from-teal-600 to-emerald-600 text-white rounded-xl font-bold"
                >
                  Submit Application
                </button>
              </div>
      ...
      {showLoanModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md space-y-4 text-slate-100 shadow-2xl">
            <h3 className="text-base font-bold text-white flex items-center gap-2">
              <Banknote className="w-5 h-5 text-teal-400" /> Apply For Employee Loan / Advance
            </h3>

            <form onSubmit={handleLoanSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-400 mb-1">Loan Amount (৳ BDT)</label>
                <input
                  type="number"
                  value={loanAmount}
              ...
              <div>
                <label className="block text-slate-400 mb-1">Repayment Tenor (Months)</label>
                <select
                  value={loanMonths}
              ...
              <div>
                <label className="block text-slate-400 mb-1">Reason for Loan Request</label>
                <textarea
                  rows={3}
                  value={loanReason}
