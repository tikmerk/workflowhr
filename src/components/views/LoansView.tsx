    ...
    <div id="loans-management-view" className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800">
          <span className="text-xs text-slate-400 block font-medium">Total Capital Disbursed</span>
          <span className="text-2xl font-black text-white mt-1 block">
            ৳{(totalDisbursed ?? 0).toLocaleString()}
          </span>
          <span className="text-[10px] text-teal-400 mt-1 block">Company-funded advances</span>
        </div>

        <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800">
          <span className="text-xs text-slate-400 block font-medium">Outstanding Principal</span>
        ...
        <div className="p-5 rounded-2xl bg-slate-900 border border-teal-500/30 bg-teal-500/5">
          <span className="text-xs text-teal-300 block font-medium">Auto Payroll Deduction</span>
          <span className="text-2xl font-black text-emerald-400 mt-1 block">100% Automated</span>
          <span className="text-[10px] text-emerald-300 mt-1 block">Subtracted in monthly payslip</span>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-800 text-slate-300">
              {filteredLoans.map((loan) => (
                <tr key={loan.id} className="hover:bg-slate-800/40 transition-colors">
                  <td className="p-3">
                    <div className="font-bold text-white">{loan.employeeName}</div>
                    <div className="text-[10px] text-slate-400">{loan.reason}</div>
                  </td>

                  <td className="p-3">{(loan.branchName || "Main Office").split("(")[0]}</td>

                  <td className="p-3 font-mono font-bold text-white">৳{(loan.amount ?? 0).toLocaleString()}</td>

                  <td className="p-3 font-mono text-teal-300">৳{(loan.monthlyEmi ?? 0).toLocaleString()} / mo</td>
                    ...
                    {loan.status === "PENDING_APPROVAL" ? (
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-emerald-600/30 text-emerald-300 hover:bg-emerald-600 hover:text-white"
                          title="Approve Loan"
                        >
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-red-600/30 text-red-300 hover:bg-red-600 hover:text-white"
                          title="Reject Loan"
                        >
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md text-slate-100 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <Banknote className="w-5 h-5 text-teal-400" /> Disburse Employee Advance Loan
              </h3>
