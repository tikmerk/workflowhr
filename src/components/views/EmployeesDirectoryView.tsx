    ...
    <div id="employees-directory-view" className="space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-500 hover:to-emerald-500 text-white text-xs font-bold rounded-xl shadow-lg shadow-teal-500/20 flex items-center gap-2 transition-all transform active:scale-95"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:hidden gap-3">
          {filteredEmployees.map((emp) => (
            ...
            >
              ...
              <div className="text-[11px] text-slate-400 space-y-1 pt-2 border-t border-slate-700/50">
                ...
                <div className="flex justify-between">
                  ...
                  {emp.faceTemplateRegistered ? (
                    ...
                    <span className="px-1.5 py-0.2 rounded text-[10px] font-bold bg-rose-500/20 text-rose-300 border border-rose-500/30 flex items-center gap-1">
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-slate-700/50">
                {onOpenDigitalIdCard && (
                  <button
                    ...
                    className="p-2 rounded-lg bg-teal-500/15 hover:bg-teal-500/25 text-teal-300 border border-teal-500/30 transition-colors cursor-pointer"
                ...
                <button
                  ...
                  className="flex-1 py-2 px-2.5 rounded-lg bg-teal-500/15 hover:bg-teal-500/25 text-teal-300 border border-teal-500/30 text-xs font-bold flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                >
            ...
            <tbody className="divide-y divide-slate-800 text-slate-300">
              {filteredEmployees.map((emp) => (
                <tr key={emp.id} className="hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="p-3">
                    <div className="font-semibold text-slate-200">{emp.designationTitle}</div>
                  ...
                  <td className="p-3">
                    <div className="font-medium text-slate-200">{(emp.branchName || "Main Office").split("(")[0]}</div>
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-1.5">
                      {emp.faceTemplateRegistered ? (
                        <span
                          className="px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 bg-emerald-500/20 text-emerald-300 border border-emerald-500/30"
                        ...
                        <span
                          className="px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 bg-rose-500/20 text-rose-300 border border-rose-500/30"
                      ...
                      {emp.boundDeviceId && (
                        <span
                          className="p-1 rounded bg-blue-500/20 text-blue-300 text-[10px] font-bold"
                  ...
                  <td className="p-3 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                      {onOpenDigitalIdCard && (
                        <button
                          onClick={() => onOpenDigitalIdCard(emp)}
                          className="p-1.5 rounded-lg bg-teal-500/10 hover:bg-teal-500/25 text-teal-400 hover:text-teal-300 border border-teal-500/30 transition-colors cursor-pointer"
                      ...
                      <button
                        onClick={() => setEnrollingEmployee(emp)}
                        className="p-1.5 rounded-lg bg-teal-500/10 hover:bg-teal-500/20 text-teal-400 hover:text-teal-300 border border-teal-500/30 transition-colors cursor-pointer"
      ...
      {selectedEmployee && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-2xl text-slate-100 shadow-2xl space-y-4 my-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
              <div className="p-3 rounded-xl bg-slate-800/60">
                <span className="text-slate-400 block text-[10px]">Official Role</span>
                <span className="font-bold text-white">{selectedEmployee.role.replace("_", " ")}</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/60">
                <span className="text-slate-400 block text-[10px]">Assigned Branch</span>
                <span className="font-bold text-white">{selectedEmployee.branchName}</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/60">
                <span className="text-slate-400 block text-[10px]">Department</span>
                <span className="font-bold text-white">{selectedEmployee.departmentName}</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/60">
                <span className="text-slate-400 block text-[10px]">Work Shift</span>
                <span className="font-bold text-teal-300">{selectedEmployee.shiftName}</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/60">
                <span className="text-slate-400 block text-[10px]">National ID (NID)</span>
                <span className="font-bold text-white">{selectedEmployee.nidNumber}</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/60">
            ...
            <div className="p-4 rounded-xl bg-slate-800/40 border border-slate-700/60 space-y-2 text-xs">
              <h4 className="font-bold text-slate-200">Biometric & Device Binding Status</h4>
              <div className="flex items-center justify-between text-slate-300">
                <span>Face Vector Recognition:</span>
                <span className="text-emerald-400 font-bold">Enrolled (98%+ Match Precision)</span>
              </div>
              <div className="flex items-center justify-between text-slate-300">
            ...
            <div className="flex flex-wrap justify-between items-center gap-2 pt-2">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-2 bg-teal-500/10 hover:bg-teal-500/20 border border-teal-500/30 text-teal-300 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
                >
                  <ScanFace className="w-4 h-4" />
                ...
                {onOpenDigitalIdCard && (
                  <button
                    ...
                    className="px-3.5 py-2 bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 text-emerald-300 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
                  >
                    <CreditCard className="w-4 h-4" />
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-3xl text-slate-100 shadow-2xl space-y-4 my-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div className="flex items-center gap-2">
                <Plus className="w-5 h-5 text-teal-400" />
                <h3 className="text-base font-bold text-white">Enroll New Employee Record</h3>
              </div>
              <button
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-slate-400 mb-1">Employee Code / ID</label>
                  <input
                    type="text"
                    value={newEmpCode}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-slate-400 mb-1">Branch Office</label>
                  <select
                    value={newBranchId}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-slate-400 mb-1">System Role (RBAC)</label>
                  <select
                    value={newRole}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-slate-400 mb-1">Basic Monthly Salary (৳ BDT)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-slate-400 mb-1">National ID (NID)</label>
                  <input
                    type="text"
                    value={newNid}
              ...
              <div>
                <label className="block text-slate-400 mb-1">Present Residential Address</label>
                <input
                  type="text"
                  value={newAddress}
