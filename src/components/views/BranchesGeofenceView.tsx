                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">{b.address}, {b.city}</p>
                    </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-slate-800/80 border border-slate-700/60 flex items-center justify-between gap-3 text-xs">
                    <div>
                      <span className="text-[10px] text-slate-400 block font-semibold">
                        {isBangla ? "ব্রাঞ্চ ম্যানেজার:" : "Branch Manager:"}
                      </span>
                      <span className="font-bold text-teal-300 text-xs block truncate max-w-[200px]">
          ...
          <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                ...
                <h3 className="text-lg font-black text-white mt-0.5">{active.name}</h3>
                <p className="text-xs text-slate-400">{active.address}, {active.city}</p>
              </div>
            ...
            <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs font-mono">
              <div>
                <span className="text-[10px] text-slate-400 block font-sans">{isBangla ? "অক্ষাংশ (Latitude):" : "Latitude:"}</span>
                <span className="font-bold text-white">{active.latitude?.toFixed(5)}° N</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 block font-sans">{isBangla ? "দ্রাঘিমাংশ (Longitude):" : "Longitude:"}</span>
                <span className="font-bold text-white">{active.longitude?.toFixed(5)}° E</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 block font-sans">{isBangla ? "অনুমোদিত ওয়াইফাই:" : "Authorized Wi-Fi:"}</span>
                <span className="font-bold text-teal-300">{active.wifiSSIDWhitelist?.[0] || "CORP_WIFI"}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs font-bold">
                ...
                <span className="text-teal-400 font-mono text-sm">{active.geofenceRadiusMeters || 150} Meters</span>
              </div>
              <input
                type="range"
                min="50"
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
              {branchEmployees.length === 0 ? (
                ...
                branchEmployees.map((emp) => (
                  <div
                    key={emp.id}
                    className="p-3 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-teal-500/40 transition-all flex items-center justify-between gap-3 text-xs"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-teal-400 font-mono">({emp.employeeCode})</span>
                        </div>
                        <span className="text-[11px] text-slate-400 block">{emp.designationTitle} • {emp.departmentName}</span>
                      </div>
                    </div>
                    ...
                    {onViewEmployee && (
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-teal-300 text-[11px] font-bold flex items-center gap-1 transition-colors shrink-0"
                      >
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-lg text-slate-100 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleBranchSubmit} className="space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-slate-400 mb-1">{isBangla ? "শাখার নাম *" : "Branch Name *"}</label>
                  <input
                    type="text"
                    value={newName}
                ...
                <div>
                  <label className="block text-slate-400 mb-1">{isBangla ? "শাখা কোড *" : "Branch Code *"}</label>
                  <input
                    type="text"
                    value={newCode}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-slate-400 mb-1">{isBangla ? "শহর *" : "City / Region *"}</label>
                  <input
                    type="text"
                    value={newCity}
                ...
                <div>
                  <label className="block text-slate-400 mb-1">{isBangla ? "ব্রাঞ্চ ম্যানেজার" : "Branch Manager"}</label>
                  <select
                    value={newManagerId}
              ...
              <div>
                <label className="block text-slate-400 mb-1">{isBangla ? "বিস্তারিত ঠিকানা *" : "Physical Address *"}</label>
                <input
                  type="text"
                  value={newAddress}
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="block text-slate-400 mb-1">{isBangla ? "অক্ষাংশ (Lat)" : "Latitude"}</label>
                  <input
                    type="number"
                    step="0.0001"
                    value={newLat}
                ...
                <div>
                  <label className="block text-slate-400 mb-1">{isBangla ? "দ্রাঘিমাংশ (Lng)" : "Longitude"}</label>
                  <input
                    type="number"
                    step="0.0001"
                    value={newLng}
                ...
                <div>
                  <label className="block text-slate-400 mb-1">{isBangla ? "রেডিয়াস (মিটার)" : "Radius (M)"}</label>
                  <input
                    type="number"
                    value={newRadius}
              ...
              <div>
                <label className="block text-slate-400 mb-1">{isBangla ? "ওয়াইফাই SSID" : "Authorized Wi-Fi SSID"}</label>
                <input
                  type="text"
                  value={newWifi}
